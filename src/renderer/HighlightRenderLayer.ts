...
export class HighlightRenderLayer extends BaseRenderLayer {
  private _state: IHighlightState;

  constructor(container: HTMLElement, zIndex: number, colors: IColorSet) {
    super(container, 'highlight', zIndex, true, colors);
    this._clearState();
  }
  ...
  public onHighlightChanged(terminal: ITerminal, start: [number, number], end: [number, number], columnSelectMode: boolean): void {
    // Highlight has not changed
    ...
    // No need to draw the highlight
    if (viewportCappedStartRow >= terminal.rows || viewportCappedEndRow < 0) {
      return;
    }

    this._ctx.fillStyle = this._colors.highlight.css;
